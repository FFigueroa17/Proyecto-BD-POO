...
namespace FormPrincipal
{
    ...
    {
        public static List<ReservaConsulta> ObtenerTodos()
        {
            ...
            {
                string query = "SELECT RE.id_reserva, EJ.portada,  EJ.nombre 'Título Ejemplar', US.nombre, RE.fecha_hora_prestamo AS 'Prestamo', RE.id_usuario, RE.id_ejemplar, RE.fecha_hora_reserva AS 'Reserva', RE.fecha_hora_devolucion AS 'Devolución' " +
                                "FROM RESERVA RE " +
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ReservaConsulta reserva = new ReservaConsulta();
                        reserva.id_reserva = Convert.ToInt32(reader["id_reserva"].ToString());
                        reserva.Ejemplar = reader["Título Ejemplar"].ToString();
                        reserva.fotografia = reader["portada"].ToString();
                        reserva.Portada = obtenerimagen(reserva.fotografia);
                        reserva.Usuario = reader["nombre"].ToString();
                        reserva.Reserva = Convert.ToDateTime(reader["Reserva"].ToString());
                        reserva.Prestamo = Convert.ToDateTime(reader["Prestamo"].ToString());
                        reserva.Devolución = Convert.ToDateTime(reader["Devolución"].ToString());
